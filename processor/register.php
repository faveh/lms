...
$refering_url = "../register/";//$_SERVER['HTTP_REFERER'];
...
$uid = md5($username.time()); // coming soon

// Check if username already exist
$userExistsQuery = mysqli_query($conn,"SELECT username FROM $lms_users WHERE username='$username'");
if(mysqli_num_rows($userExistsQuery) != 0) {
	header("location: $refering_url?msg=exists1");
}
// Check if email already exist
$emailExistsQuery = mysqli_query($conn,"SELECT email FROM $lms_users WHERE email='$email'");
if(mysqli_num_rows($emailExistsQuery) != 0) {
	header("location: $refering_url?msg=exists2");
}
// Check if mobile number already exist
$mobilenoExistsQuery = mysqli_query($conn,"SELECT mob_no FROM $lms_users WHERE mob_no='$mobileno'");
...
mysqli_query($conn,"ALTER TABLE $lms_users AUTO_INCREMENT = 1;"); // reset auto increment

$sql= "INSERT INTO $lms_users (username,email,password,name,mob_no,user_uid) 
VALUES('$username','$email','$password','$name','$mobileno','$uid')";
